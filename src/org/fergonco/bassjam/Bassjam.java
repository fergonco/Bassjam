...
public class Bassjam {
	...
	public static void main(String[] args) throws IOException {
		...
		Duration[] rythm = newRythm();
		StringBuilder patternString = new StringBuilder("X[Volume]=10 T").append(tempo).append(" V0 I[Acoustic_Bass] ");
		StringBuilder bassPattern = patternString;
		for (int i = 0; i < chordProgression.length; i++) {
			notes = chordProgression[i];
			for (Duration duration : rythm) {
				String note = notes[(int) (Math.random() * notes.length)];
				bassPattern.append(note).append("2").append(duration.getDuration()).append(" ");
			}
			bassPattern.append("| ");
		}
		PatternProducer pattern = new Pattern(bassPattern.toString()).repeat(400);
		String chordsPattern = getChordsPattern(chordProgression, tempo);
		PatternProducer chords = new Pattern(chordsPattern).repeat(400);
		play(pattern, chords);
		JFrame f = new JFrame("Bassjam");
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		JTextArea txtCode = new JTextArea(bassPattern.toString() + "\n" + chordsPattern);
		f.getContentPane().add(txtCode, BorderLayout.CENTER);
		f.pack();
		txtCode.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 10:
					...
					PatternProducer[] patterns = parse(code);
	...
	private static PatternProducer[] parse(String code) {
		String[] patternStrings = code.split("\n");
		PatternProducer[] patterns = new PatternProducer[patternStrings.length];
		for (int i = 0; i < patterns.length; i++) {
			String patternString = patternStrings[i];
			patterns[i] = new Pattern(patternString).repeat(400);
		}
		return patterns;
	}

	private static void play(PatternProducer... patterns) {
		player = new Player();
	...
	private static String getChordsPattern(String[][] chordProgression, int tempo) {
		StringBuilder chordProgressionString = new StringBuilder("X[Volume]=1 T").append(tempo).append(" V1 I[Choir] ");
		for (String[] chord : chordProgression) {
			for (String note : chord) {
				chordProgressionString.append(note).append("6w+");
			}
			chordProgressionString.setLength(chordProgressionString.length() - 1);
			chordProgressionString.append(" ");
		}
		return chordProgressionString.toString();
	}

	private static Duration[] newRythm() {
		Duration[] durations = new Duration[] { new Duration("q"), new Duration("i"), new Duration("i."),
				new Duration("s") };
		ArrayList<Duration> ret = new ArrayList<>();
		int length = 0;
		while (length < 16) {
			Duration duration;
			do {
				duration = durations[(int) (Math.random() * durations.length)];
			} while (length + duration.getSixteenths() > 16);
			length += duration.getSixteenths();
			ret.add(duration);
		}

		return ret.toArray(new Duration[ret.size()]);
	}
}
// public class MidiDictionary {
// public static Map<Byte, String> INSTRUMENT_BYTE_TO_STRING = new HashMap<Byte,
// String>() {{
// put((byte)0, "Piano");
...
// }};
//
// public static Map<String, Byte> INSTRUMENT_STRING_TO_BYTE = new
// HashMap<String, Byte>() {{
// put("PIANO", (byte)0);
...
// }};
//
// public static Map<Integer, String> TEMPO_INT_TO_STRING = new HashMap<Integer,
// String>() {{
// put(40, "GRAVE");
// put(45, "LARGO");
// put(50, "LARGHETTO");
...
// }};
//
// public static Map<String, Integer> TEMPO_STRING_TO_INT = new HashMap<String,
// Integer>() {{
// put("GRAVE", 40);
// put("LARGO", 45);
// put("LARGHETTO", 50);
...
// }};
//
// public static Map<Integer, String> CONTROLLER_INT_TO_STRING = new
...
// }};
//
// public static Map<String, Integer> CONTROLLER_STRING_TO_INT = new
